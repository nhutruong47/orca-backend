...
function hideAiRoadmapText(text?: string | null) {
    const value = (text || '').trim();
    const lower = value.toLowerCase();
    const isRawAiRoadmap = lower.includes('chào anh/chị')
        || lower.includes('chao anh/chi')
        || lower.includes('roadmap')
        || lower.includes('bảng roadmap')
        || lower.includes('bang roadmap')
        || lower.includes('báº£ng roadmap')
        || (lower.includes('stt') && (lower.includes('công việc') || lower.includes('cÃ´ng viá»‡c')));

    return isRawAiRoadmap ? '' : value;
}
                ...
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #edf2f7', background: 'linear-gradient(180deg, #fffaf3, #fff)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, minWidth: 0 }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                            ...
                            {hideAiRoadmapText(latestGoal.outputTarget || latestGoal.rawInstruction) && (
                                <div style={{ marginTop: 6, color: '#64748b', fontSize: 13 }}>{hideAiRoadmapText(latestGoal.outputTarget || latestGoal.rawInstruction)}</div>
                            )}
                        </div>
                    </div>
